...
Vector v=jf.getValue("select title, movie,year_of_release,singers,Youtube_link,Audio_link,Music_director,mainCategory from songs2 order by FIELD(singers,(select singer from getPref where userid='"+session.getAttribute("userid").toString().trim()+"' order by cnt desc limit 1)) desc, FIELD(Music_director,(select music_director from getPref where userid='"+session.getAttribute("userid").toString().trim()+"' order by cnt desc limit 1)) desc ,FIELD(mainCategory,(select category from getPref where userid='"+session.getAttribute("userid").toString().trim()+"' order by cnt desc limit 1)) desc, songId desc limit 100", 8);
%>
